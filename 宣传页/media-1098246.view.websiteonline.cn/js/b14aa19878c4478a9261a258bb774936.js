...
function layer_article_category_hs9_func(params){
	...
	$(document).ready(function(){
		...
		/*function getSubMenuHoverCss<?php echo $layer_id;?>(css_pro,regex,isLi)
		{
			<?php
			if(Toolkit::editMode()){
			?>
			var navStyle = $.trim($("#datastys_<?php echo $layer_id;?>").text());
			<?php }else{ ?>
			var navStyle = $.trim($("#<?php echo $layer_id;?>").data("datastys_"));
			<?php } ?>
			if(navStyle.length > 0)
			{
				var patt1 =new RegExp(regex,'i');
				...
				if(Toolkit::editMode()){
				?>
				navStyle = $.trim($("#datasty_<?php echo $layer_id;?>").text());
				<?php }else{ ?>
				navStyle = $.trim($("#<?php echo $layer_id;?>").data("datasty_"));
				<?php } ?>
